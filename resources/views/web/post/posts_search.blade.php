            ...
            <div class="post__container ">
                <div class="post__sub-container-one ">
                    <div class="post__image-container ">
                        <img src="{{ url('/storage/uploades/posts') }}/{{ $post->image }}" class="post__image"
                            alt="">
                    </div>
...
@if (!empty($skills))
    @foreach ($skills as $skill)
        @foreach ($skill->posts as $key => $post)
                        ...
                        <div class="post__many-detail ">
                            ...
                            <div class="post__poster-detail ">
                                <div class="post__avatar">
                                    ...
                                    <img src="{{ url('/storage/uploades/imageProfile') }}/{{ $skill->posts[$key]->user->image }}"
                                        class="post__poster-avatar" alt="">
                            ...
                            <div class="post__actions ">
                                ...
                                @if ($skill->posts[$key]->user->id == Auth::id() && Auth::user()->confirm_account == 1)
                                    <a href="{{ route('post.edit', $post->id) }}"><button
                                            class="btn__apply">edit</button></a>
                                    <form method="post" action="{{ route('post.destroy', $post->id) }}">
                                        {{ csrf_field() }}
                                ...
                                @elseif(Auth::user()->role_id ==3 && Auth::user()->confirm_account
                                    == 1)
                                    @php
                                        $checkPost = App\Models\PostRequest::where('post_id', $post->id)
                                            ->where('user_id', Auth::id())
